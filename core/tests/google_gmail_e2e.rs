...
//!
//! To run these tests:
//! 1. Generate a refresh token with Gmail API scopes:
//!    - Required scope: https://www.googleapis.com/auth/gmail.readonly
//!    - Or for full access: https://www.googleapis.com/auth/gmail.modify
//! 2. Set GOOGLE_TEST_REFRESH_TOKEN environment variable with the refresh token
//! 3. Ensure auth.ariata.com is accessible
//! 4. Ensure the test account has emails in INBOX or SENT from the last 30 days
//! 5. Run with: cargo test google_gmail --test google_gmail_e2e -- --ignored --nocapture
//!
//! Note: If you only have Calendar API scopes, the test will fail with 0 messages synced.
...
async fn test_google_gmail_real_oauth_e2e() -> Result<()> {
    ...
    // Note: Gmail API interprets multiple labelIds as AND (not OR)
    // So ["INBOX", "SENT"] means "messages with BOTH labels" which is usually 0
    // For OR behavior, use just one label or use query parameter
    ...
    let mut gmail_sync = GoogleGmailSync::new(source_id, fixture.db.clone(), token_manager.clone());

    // Load config from database
    gmail_sync.load_config().await?;
    ...
    assert!(
        ...
        "Should have synced at least some messages. \
         \n\nPossible causes:\
         \n1. OAuth token may not have Gmail API scopes (only Calendar scopes)\
         \n2. No messages in the account matching the filters (INBOX, SENT, last 30 days)\
         \n3. Gmail API is returning empty responses\
         \n\nTo fix:\
         \n- Ensure GOOGLE_TEST_REFRESH_TOKEN has scope: https://www.googleapis.com/auth/gmail.readonly\
         \n- Check that the test account has emails in INBOX or SENT from the last 30 days"
    );
    ...
    let mut inbox_sync = GoogleGmailSync::new(source_id, fixture.db.clone(), token_manager.clone());
    inbox_sync.load_config().await?;
