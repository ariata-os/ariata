...
use crate::{
    ...
    sources::base::{
        DataSource, GoogleSyncToken, HealthStatus, SyncCursor, SyncLogger, SyncMode, SyncResult,
        TypedCursor,
    },
};
...
impl GoogleGmailSync {
    ...
    pub fn new(source_id: Uuid, db: PgPool, token_manager: Arc<TokenManager>) -> Self {
        let client = GoogleClient::with_api(source_id, token_manager.clone(), "gmail", "v1");
    ...
    pub async fn load_config(&mut self) -> Result<()> {
        ...
        if let Some((config_json,)) = result {
            tracing::debug!(config_json = %config_json, "Loading config from database");
            if let Ok(config) = GoogleGmailConfig::from_json(&config_json) {
                tracing::info!(
                    ...
                    max_messages = %config.max_messages_per_sync,
                    sync_mode = ?config.sync_mode,
                    "Loaded Gmail config"
                );
                self.config = config;
            } else {
                tracing::warn!("Failed to parse config JSON, using defaults");
            }
        } else {
            tracing::warn!("No config found in database, using defaults");
        }
    ...
    pub async fn sync_gmail(&self) -> Result<SyncResult> {
        let started_at = Utc::now();
        let logger = SyncLogger::new(self.db.clone());

        // Get last history ID to determine mode
        let last_token = self.get_last_history_id().await?;
        let sync_mode = if let Some(token) = last_token {
    ...
    async fn sync_internal(&self, sync_mode: &SyncMode) -> Result<SyncResult> {
        ...
        match sync_mode {
            SyncMode::Incremental { cursor } if cursor.is_some() => {
                // Use history API for incremental sync
                let history_id = cursor.as_ref().unwrap();
    ...
    async fn sync_incremental(
        ...
        if let Some(new_history_id) = response.history_id {
            latest_history_id = new_history_id.clone();
            let token = GoogleSyncToken(new_history_id);
            self.save_history_id(&token).await?;
        }
    ...
    async fn sync_messages_full(&self) -> Result<(usize, usize, usize, Option<String>)> {
        ...
        // Build base query parameters (without page token)
        let mut base_params = vec![("maxResults", self.config.max_messages_per_sync.to_string())];

        // Add label filters
        for label in &self.config.label_ids {
            base_params.push(("labelIds", label.clone()));
        }
        ...
        tracing::debug!(
            query = %query,
            sync_window_days = %self.config.sync_window_days,
            "Built Gmail search query"
        );
        if !query.is_empty() {
            base_params.push(("q", query));
        }

        if self.config.include_spam_trash {
            base_params.push(("includeSpamTrash", "true".to_string()));
        }
        ...
        loop {
            ...
            tracing::debug!(
                page = page_count,
                message_count = ?response.messages.as_ref().map(|m| m.len()),
                next_page_token = ?response.next_page_token,
                result_size_estimate = ?response.result_size_estimate,
                "Received messages list response"
            );
        ...
        tracing::info!(
            ...
            "Completed paginated message sync"
        );
        ...
        if let Ok(profile) = self.get_profile().await {
            if let Some(history_id) = profile.get("historyId").and_then(|v| v.as_str()) {
                ...
                let token = GoogleSyncToken(history_id.to_string());
                self.save_history_id(&token).await?;
            }
        }
    ...
    async fn sync_threads_full(&self) -> Result<(usize, usize, usize, Option<String>)> {
        ...
        loop {
            ...
            tracing::debug!(
                page = page_count,
                thread_count = ?response.threads.as_ref().map(|t| t.len()),
                next_page_token = ?response.next_page_token,
                result_size_estimate = ?response.result_size_estimate,
                "Received threads list response"
            );
    ...
    /// Get the last history ID from the database (typed cursor)
    async fn get_last_history_id(&self) -> Result<Option<GoogleSyncToken>> {
        ...
        if let Some(json) = row.0 {
            ...
            Ok(Some(typed.decode::<GoogleSyncToken>()?))
        } else {
            Ok(None)
        }
    }

    /// Save the history ID to the database (typed cursor)
    async fn save_history_id(&self, token: &GoogleSyncToken) -> Result<()> {
...
#[async_trait]
impl DataSource for GoogleGmailSync {
    ...
    where
        Self: Sized,
    {
        GoogleGmailConfig::from_json(config)
            .map_err(|e| Error::Configuration(format!("Invalid GoogleGmailConfig: {e}")))?;
        Ok(())
    }
    ...
    async fn sync(&self, mode: &SyncMode) -> Result<SyncResult> {
        ...
        tracing::info!(mode = ?mode, "Starting Gmail sync via DataSource");
    ...
    async fn health_check(&self) -> Result<HealthStatus> {
        ...
        match token_check {
            Ok(_) => Ok(HealthStatus {
                is_healthy: true,
                message: "Gmail OAuth token is valid".to_string(),
                last_check: Utc::now(),
            ...
            Err(e) => Ok(HealthStatus {
                is_healthy: false,
                message: format!("Gmail OAuth token error: {e}"),
                last_check: Utc::now(),
                last_successful_sync: last_sync,
                error_count: 1,
            }),
    ...
    fn sync_schedule(&self) -> String {
        // Sync every 5 minutes (same as Calendar)
        "0 */5 * * * *".to_string()
    }
        ...
        match self.token_manager.get_valid_token(self.source_id).await {
            ...
            Err(e) => {
                tracing::warn!(
                    ...
                    error = %e,
                    "Gmail sync skipped: no valid OAuth token"
                );
                false
            }
        }
